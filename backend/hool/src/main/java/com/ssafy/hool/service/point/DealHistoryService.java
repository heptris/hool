...
public class DealHistoryService {
    ...
    public DealHistoryResponseDto makeDeal(DealHistoryCreateDto dealHistoryCreateDto, Long memberId){
        ...
        Member_emoji isExistEmoji = memberEmojiRepository.findByMemberIdAndEmojiId(memberId, emoji.getId());

        if(isExistEmoji == null){
            ...
            if(currentBuyerPoint >= 0){
                pointHistoryCreateDto = new PointHistoryCreateDto(emoji.getName() + "이모지 구매", -dealHistory.getDealPoint(), currentBuyerPoint, PointType.DEAL);
            ...
            pointHistoryCreateDto = new PointHistoryCreateDto(emoji.getName() + "이모지 판매", dealHistory.getDealPoint(), currentSellerPoint, PointType.DEAL);
...
//            emojiShopRankingService.updateRanking();
            ...
            throw new CustomException(ALREADY_HAVE_EMOJI);
        }
    }
}
